...
{
    public partial class ConfigScreenR2 : IronPanel.InformationPanel
    {
        ...
        bool[] AlarmResult = new bool[200];

        //전력 값 및 가스 값의 이전 데이타를 저장하는 변수
        double acc_electric_before = -1;
        double acc_gas_before = -1;
       

        public ConfigScreenR2()
        {
            InitializeComponent();           
        }
        ...
        private void CallBack_AlarmTagEvent(int notificationID, object[] value)
        {
            try
            {
                ...
                if (AlarmResult[notificationID] == data)
                {
                    return;
                }
                else
                {
                    AlarmResult[notificationID] = data;
                }
            ...
            try
            {
                ...
                if (notificationID == diHandle[0])  // scrap weight send bit
                {
                    if (data)
                    {
                        ...
                        IronControls.API.OutputDebugString("Read Success at Event, Value = " + scrap_meas.ToString());

                        timer_scrap_delay.Enabled = true;

                        /****
                        long scrap_meas = Convert.ToInt32(opcClient2.ReadAny("PLC-XGK:D00514").ToString().Trim());

                        IronControls.API.OutputDebugString(" Read Success, Value = " + scrap_meas.ToString());

                        double d_scrap_factor = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.scrap_per_kg").ToString().Trim()); // PLC 스크랩 중량을 실제값으로 환산하는 팩터
                        ...
                        MDB_ExecuteNonQuery(fullFilename, "INSERT INTO LOG_FULLDATA (시스템시간, 작업일자, 작업시간, 스크랩, 스크랩_PLC, 스크랩사용량, 적산스크랩량)  VALUES ( '"
                           + ndt.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + wdt.ToString("yyyy-MM-dd") + "', '" + wdt.ToString("yyyy-MM-dd HH:mm:ss") + "', '스크랩계량',"
                           + scrap_meas + "," + d_scrap_per_min.ToString("0.0").Trim() + "," + "0.0" + ")");

                        IronControls.API.OutputDebugString("INSERT INTO LOG_FULLDATA (시스템시간, 작업일자, 작업시간, 스크랩, 스크랩_PLC, 스크랩사용량, 적산스크랩량)  VALUES ( '"
                           + ndt.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + wdt.ToString("yyyy-MM-dd") + "', '" + wdt.ToString("yyyy-MM-dd HH:mm:ss") + "', '스크랩계량',"
                           + scrap_meas.ToString() + "," + d_scrap_per_min.ToString("0.0").Trim() + "," + "0.0" + ")");
                ...
                else if (notificationID == diHandle[1]) // Ingot weight send bit
                {
                    if (data)
                    {
                        ...
                        IronControls.API.OutputDebugString("Read Success at Event, Value = " + ingot_meas.ToString());

                        timer_ingot_delay.Enabled = true;

                        /****
                        long ingot_meas = Convert.ToInt32(opcClient2.ReadAny("PLC-XGK:D00625").ToString().Trim());

                        IronControls.API.OutputDebugString(" Read Success, Value = " + ingot_meas.ToString());
                        ...
                        + ndt.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + wdt.ToString("yyyy-MM-dd") + "', '" + wdt.ToString("yyyy-MM-dd HH:mm:ss") + "', '인고트계량',"
                ...
                //ksyu 무게 값 변경 시 바로 생산현황에 나타 나도록 하기 위해서 추가
        ...
        private void timer_scrap_delay_Tick(object sender, EventArgs e)
        {
            string db_pathrootname;
            string db_filename_Header = "DATALOG-EEMS-M1";
            string fullFilename;
            ...
            IronControls.API.OutputDebugString("Read Success for log, Value = " + scrap_meas.ToString());
            ...
            CurrentData_Update();
            timer_scrap_delay.Enabled = false;
        }

        private void timer_ingot_delay_Tick(object sender, EventArgs e)
        {
            string db_pathrootname;
            string db_filename_Header = "DATALOG-EEMS-M1";
            string fullFilename;
            ...
            IronControls.API.OutputDebugString("Read Success for log, Value = " + ingot_meas.ToString());
            ...
            CurrentData_Update();
            timer_ingot_delay.Enabled = false;
        }
        ...
        {          
            ...
            try
            {
                IronControls.API.OutputDebugString("Query String = " + strSQL);
                conn.Open();
        ...
        public static bool CreateMDBTables(string fullFilename)
        {
            try
            {
                //ksyu 테이블 만들다 오류 발생 시 빠져 나오도록 수정
                //LOG_FULLDATA 제외하고 테이블 만드는 부분 삭제 예정
        ...
        //ksyu 로그 파일 삭제 기본값은 Enable true 상태에 기간 1년
        ...
        private void timer_work_data_Tick(object sender, EventArgs e)
        {
            ...
            if (ndt.Month != bdt.Month || !File.Exists(fullFilename))
            {
                ...
                //로그 삭제 관련 설정 값을 정상적으로 읽어 왔는지 확인
            ...
            // 일이 경과했으면 새로운 ..  시간이 경과했으면 새로운 ..  분이 경과했으면 Periodic Data[분단위] 저장
        ...
        public void Log_Per_Minute(string fullFilename, string sysTime, string workDay, string workElapsedTime)
        {
            ...
            try
            {
                //ksyu 전력 값 및 가스 값이 이전 값이 있는지 확인하고 없으면 로그에서 얻어오도록 변경
                if (acc_electric_before < 0 || acc_gas_before < 0)
                {
                ...
                {
                    //ksyu PLC 에서 값을 가져오지 못 하는 경우 (Null값이 나와 Convert 중 에러 발생) 저장 하지 않고 함수에서 나오도록 변경
                    //ksyu PLC 제외하고 테스트 위해 값을 주는 부분은 남겨 두며 리턴을 막고 테스트 해야 함.
                    acc_gas = 0;
                    acc_electric = 0;
                    plc_temp_tower = 0;
               ...
               double d_pulse_factor = 1;
               double d_watt_factor = 1;
               double d_temp_tower = 1;
               double d_temp_furnace = 1;
               double d_temp_tower_set = 1;
               double d_temp_furnace_set = 1;

               try
               {
                   d_pulse_factor = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.pulse_per_gas").ToString().Trim()); // PLC 가스값을 실제값으로 환산하는 팩터
                   d_watt_factor = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.plc_watt_per_pc_watt").ToString().Trim());  // PLC 전력값을 실제값으로 환산하는 팩터
                   d_temp_tower = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.tower_temp_factor").ToString().Trim()); // PLC 온도를 소수점 환산하는 팩터
                   d_temp_furnace = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.furnace_temp_factor").ToString().Trim());  // PLC 온도를 소수점 환산하는 팩터
                   d_temp_tower_set = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.tower_temp_factor_set").ToString().Trim()); // PLC 온도를 소수점 환산하는 팩터
                   d_temp_furnace_set = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.furnace_temp_factor_set").ToString().Trim());  // PLC 온도를 소수점 환산하는 팩터
               }
               catch
               {
                   d_pulse_factor = 1;
                ...
                //ksyu 전력 값이 이전 보다 작을 경우 (전력계 리셋) 처리 추가
                if (gap_electric < 0)
                {
                ...
                //ksyu 가스 값이 이전 보다 작을 경우 (가스량 리셋) 처리 추가
                if (gap_gasflow < 0)
                {
                ...
                //KSYU INSERT 후 이전 데이타에 현재값 치환
                acc_electric_before = acc_electric;
                acc_gas_before = acc_gas;
        ...
        public void CurrentData_Update()
        {
            ...
            try
            {
                /////////////////////////////////////////////////////////////////////////////////////
                // ksyu
                // 스크랩 인고트 전력 가스를 따로 읽던것을 시/일/월 에서 각각 한번 읽도록 변경
                // 스크랩 인고트 환산 단위값을 config에서 읽어 계산하도록 변경
                // 전력 및 가스 는 환산한 사용량을 읽어오기에 환산 하는 계산을 안하도록 변경
                // 스크랩 인고트 비율 계산 추가
                // 총 중량 계산시 스크랩 인고트 에서 환산 한 값을 사용하도록 변경( 이미 환산 한 값을 사용하기에 다시 환산하는 계산은 제거)
                // 데이타가 없을 경우(select시 읽어온 count가  > 0) 0으로 값을 보내는 부분 추가
                // 읽어 왔을 때 table이나 row가 없을 때 모든 값을 0으로 보내는 부분 수정(나누어서 읽지 않기에 한번 만 체크)
                //////////////////////////////////////////////////////////////////////////////////////
                ...
                // 현재 시간값 업데이트
                tablename = "LOG_FULLDATA";
                strQuery = "SELECT SUM(스크랩_PLC), SUM(인고트_PLC),  SUM(전력사용량), SUM(가스사용량)  AS 분당투입량 FROM " + tablename + " WHERE 작업시간 LIKE '" +
                ...
                if (ds1.Tables.Count > 0)
                {
                    if (Convert.ToUInt16(ds1.Tables[0].Rows.Count) > 0)
                    {
                        ...
                        dFactor = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.scrap_per_kg").ToString().Trim());
                        ...
                        dOrigin = Convert.ToDouble(ds1.Tables[0].Rows[0][1]);
                        dFactor = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.ingot_per_kg").ToString().Trim());
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Put_All_Per_Hour", dOrigin);
                            FullValue = dOrigin;
                        ...
                        dOrigin = Convert.ToDouble(ds1.Tables[0].Rows[0][2]);
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Watt_Per_Hour", dOrigin);
                        }
               
                        dOrigin = Convert.ToDouble(ds1.Tables[0].Rows[0][3]);
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Gas_Per_Hour", dOrigin);
                            Temp_Gas = dOrigin;
                        }

                        //m3/ton
                        if (Temp_Gas < 0.01 || FullValue < 0.01) dOrigin = 0.0;
                        else dOrigin = Temp_Gas / (FullValue / 1000);
                    ...
                    {
                        ironSANClient1.WriteData("AO.EEMS-R1.Scrap_Per_Hour", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Ingot_Per_Hour", 0);                      
                        ironSANClient1.WriteData("AO.EEMS-R1.Put_All_Per_Hour", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Scrap_Per_HourPe", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Ingot_Per_HourPe", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Watt_Per_Hour", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Gas_Per_Hour", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.m3Ton_Per_Hour", 0);
                    }
                }
                else
                {
                ...
                // 현재 일자깂 업데이트
                tablename = "LOG_FULLDATA";
                strQuery = "SELECT SUM(스크랩_PLC), SUM(인고트_PLC),  SUM(전력사용량), SUM(가스사용량)  AS 시간당투입량 FROM " + tablename + " WHERE 작업시간 LIKE '" +
                ...
                if (ds1.Tables.Count > 0)
                {
                    if (Convert.ToUInt16(ds1.Tables[0].Rows.Count) > 0)
                    {
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Put_All_Per_Day", dOrigin);
                            FullValue = dOrigin;
                            SWePer = (ScrValue / FullValue) * 100;
                            IWePer = (IgtValue / FullValue) * 100;
                        }
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Watt_Per_Day", dOrigin);
                        }
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Gas_Per_Day", dOrigin);
                            Temp_Gas = dOrigin;
                        }
                    ...
                    {
                        ironSANClient1.WriteData("AO.EEMS-R1.Scrap_Per_Day", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Ingot_Per_Day", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Put_All_Per_Day", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Scrap_Per_DayPe", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Ingot_Per_DayPe", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Watt_Per_Day", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Gas_Per_Day", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.m3Ton_Per_Day", 0);
                    }
                }
                else
                {
                ...
                // 현재 월값 업데이트
                tablename = "LOG_FULLDATA";
                strQuery = "SELECT SUM(스크랩_PLC), SUM(인고트_PLC),  SUM(전력사용량), SUM(가스사용량)  AS 일별투입량 FROM " + tablename + " WHERE 작업시간 LIKE '" +
                ...
                if (ds1.Tables.Count > 0)
                {
                    if (Convert.ToUInt16(ds1.Tables[0].Rows.Count) > 0)
                    {
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Put_All_Per_Month", dOrigin);
                            FullValue = dOrigin;
                            SWePer = (ScrValue / FullValue) * 100;
                            IWePer = (IgtValue / FullValue) * 100;
                        }
                        ...
                        {
                            ironSANClient1.WriteData("AO.EEMS-R1.Gas_Per_Month", dOrigin);
                            Temp_Gas = dOrigin;
                        }
                    ...
                    {
                        ironSANClient1.WriteData("AO.EEMS-R1.Scrap_Per_Month", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Ingot_Per_Month", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Put_All_Per_Month", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Scrap_Per_MonthPe", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Ingot_Per_MonthPe", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.Gas_Per_Month", 0);
                        ironSANClient1.WriteData("AO.EEMS-R1.m3Ton_Per_Month", 0);
                    }
                }
                else
                {
                ...
                //ksyu 연간 현황 사용 코드 정리 안되있으며 테스트 해보지 않았음.
                //////////////////////////////////////////////////////////////////////////////
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            double scrap_meas = analogBox22.Data;
            double ingot_meas = analogBox21.Data;

            double d_scrap_factor = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.scrap_per_kg").ToString().Trim()); // PLC 스크랩 중량을 실제값으로 환산하는 팩터
            double d_Ingot_factor = Convert.ToDouble(ironSANClient1.ReadAny("AO.EEMS-R1.ingot_per_kg").ToString().Trim()); // PLC 스크랩 중량을 실제값으로 환산하는 팩터
            ...
            MDB_ExecuteNonQuery(fullFilename, "INSERT INTO LOG_FULLDATA (시스템시간, 작업일자, 작업시간, 스크랩, 스크랩_PLC, 스크랩사용량, 적산스크랩량,인고트, 인고트_PLC, 인고트사용량, 적산인고트량)  VALUES ( '"
               + ndt.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + wdt.ToString("yyyy-MM-dd") + "', '" + wdt.ToString("yyyy-MM-dd HH:mm:ss") + "', '추가 중량',"
               + d_scrap_per_min.ToString("0.0").Trim() + "," + scrap_meas.ToString("0.0").Trim() + "," + "0.0" + ", '추가 중량'," + d_ingot_per_min.ToString("0.0").Trim() + "," + ingot_meas.ToString("0.0").Trim() + "," + "0.0" + ")");
