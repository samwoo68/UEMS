...
{
    public partial class TPLogDataScreen : IronPanel.InformationPanel
    {
        ...
        private void LogDataScreen_Load(Object sender, EventArgs e)
        {
            ...
            selListViewC1.ListViewItem(dt, "TPLog_Config_LV.xml", this.ProjectPath);
            selGraphViewC1.GraphViewItem(dt, "TPLog_Config.xml", this.ProjectPath);
        ...
        {
            //Bitmap myImage = global::SampleScreen.Properties.Resources.UBI;
        ...
        private DataTable Get_Data()
        {
            ...
            try
            {
                ...
                strQuery = " SELECT " + str_group_format + " AS 작업일시, " +
                           " AVG(타워온도설정값_PLC) AS 타워온도설정값_PLC_AVG, SUM(타워온도설정값) AS 타워온도설정값_AVG, " +
                           " MIN(타워온도설정값_PLC) AS 타워온도설정값_PLC_MIN, SUM(타워온도설정값) AS 타워온도설정값_MIN, " +
                           " MAX(타워온도설정값_PLC) AS 타워온도설정값_PLC_MAX, SUM(타워온도설정값) AS 타워온도설정값_MAX, " +
                           " AVG(타워온도_PLC) AS 타워온도_PLC_AVG, AVG(타워온도값) AS 타워온도값_AVG, " +
                           " MIN(타워온도_PLC) AS 타워온도_PLC_MIN, MIN(타워온도값) AS 타워온도값_MIN, " +
                           " MAX(타워온도_PLC) AS 타워온도_PLC_MAX, MAX(타워온도값) AS 타워온도값_MAX, " +
                           " AVG(용탕온도설정값_PLC) AS 용탕온도설정값_PLC_AVG, AVG(용탕온도설정값) AS 용탕온도설정값_AVG, " +
                           " MIN(용탕온도설정값_PLC) AS 용탕온도설정값_PLC_MIN, MIN(용탕온도설정값) AS 용탕온도설정값_MIN, " +
                           " MAX(용탕온도설정값_PLC) AS 용탕온도설정값_PLC_MAX, MAX(용탕온도설정값) AS 용탕온도설정값_MAX, " +
                           " AVG(용탕온도_PLC) AS 용탕온도_PLC_AVG, AVG(용탕온도값) AS 용탕온도값_AVG, " +
                           " MIN(용탕온도_PLC) AS 용탕온도_PLC_MIN, MIN(용탕온도값) AS 용탕온도값_MIN, " +
                           " MAX(용탕온도_PLC) AS 용탕온도_PLC_MAX, MAX(용탕온도값) AS 용탕온도값_MAX  " +
                ...
                strQuery = " SELECT 'SUM' AS 구분, " +
                    " SUM(타워온도설정값_PLC) AS 타워온도설정값_PLC_AVG, SUM(타워온도설정값) AS 타워온도설정값_AVG, " +
                    " MIN(타워온도설정값_PLC) AS 타워온도설정값_PLC_MIN, MIN(타워온도설정값) AS 타워온도설정값_MIN, " +
                    " MAX(타워온도설정값_PLC) AS 타워온도설정값_PLC_MAX, MAX(타워온도설정값) AS 타워온도설정값_MAX, " +
                    " SUM(타워온도_PLC) AS 타워온도_PLC_AVG, SUM(타워온도값) AS 타워온도값_AVG, " +
                    " MIN(타워온도_PLC) AS 타워온도_PLC_MIN, MIN(타워온도값) AS 타워온도값_MIN, " +
                    " MAX(타워온도_PLC) AS 타워온도_PLC_MAX, MAX(타워온도값) AS 타워온도값_MAX, " +
                    " SUM(용탕온도설정값_PLC) AS 용탕온도설정값_PLC_AVG, SUM(용탕온도설정값) AS 용탕온도설정값_AVG, " +
                    " MIN(용탕온도설정값_PLC) AS 용탕온도설정값_PLC_MIN, MIN(용탕온도설정값) AS 용탕온도설정값_MIN, " +
                    " MAX(용탕온도설정값_PLC) AS 용탕온도설정값_PLC_MAX, MAX(용탕온도설정값) AS 용탕온도설정값_MAX, " +
                    " SUM(용탕온도_PLC) AS 용탕온도_PLC_AVG, SUM(용탕온도값) AS 용탕온도값_AVG, " +
                    " MIN(용탕온도_PLC) AS 용탕온도_PLC_MIN, MIN(용탕온도값) AS 용탕온도값_MIN, " +
                    " MAX(용탕온도_PLC) AS 용탕온도_PLC_MAX, MAX(용탕온도값) AS 용탕온도값_MAX  " +
                    //" SUM(타워온도설정값_PLC) AS 타워온도설정값_PLC, SUM(타워온도설정값) AS 타워온도설정값, " +
                    //" SUM(타워온도_PLC) AS 타워온도_PLC, SUM(타워온도값) AS 타워온도값, " +
                ...
                if (ds8.Tables.Count > 0)
                {
                    ...
                    if (ds8.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 1; i < ds8.Tables[tablename].Columns.Count; i++)
                        {
                            ...
                            {
                                if (i == 1 || i == 2 || i == 7 || i == 8 || i == 13 || i == 14 || i == 19 || i == 20 )
                                {
                                    TempDR[i] = ds8.Tables[tablename].Compute("SUM(" + ds8.Tables[0].Columns[i].Caption + ")", "1=1");
                                    double AVGSum = double.Parse(TempDR[i].ToString());
                                    AVGSum = AVGSum / AVG_Count;
                                    TempDR[i] = AVGSum.ToString("F2");
                                }
                                else if (i == 3 || i == 4 || i == 9 || i == 10 || i == 15 || i == 16 || i == 21 || i == 22)
                                {
                                    TempDR[i] = ds8.Tables[tablename].Compute("MIN(" + ds8.Tables[0].Columns[i].Caption + ")", "1=1");
                                }
                                else
                                {
